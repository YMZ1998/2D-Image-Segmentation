...
def convert(json_path: Path, output_path: Path) -> None:
    ...
    shapes = sorted(
        ...
        key=lambda shape: LABEL_TO_ID.get(shape.get("label", ""), 0),
    )
    for shape in shapes:
        ...
        draw.polygon(points, fill=LABEL_TO_ID[label])
...
def main() -> None:
    ...
    mapping = ", ".join(["background=0", *(f"{name}={value}" for name, value in LABEL_TO_ID.items())])
